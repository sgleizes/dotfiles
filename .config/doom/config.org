...
#+begin_src elisp
(after! all-the-icons
  (defun maker/doom-init-all-the-icons-fonts-h ()
...
(advice-add #'doom-init-all-the-icons-fonts-h :override #'maker/doom-init-all-the-icons-fonts-h)
#+end_src
...
(use-package! elfeed
  ...
  (setq elfeed-search-remain-on-entry 't
        elfeed-search-sort-function #'maker/elfeed-search-entry<
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-search-selected (&optional ignore-region-p)
      ...
      (let ((start (if use-region (region-beginning) (point)))
        ...
        (cl-loop for line from (line-number-at-pos start)
                 ...
                 collect it into selected
                                     ...
                                     (car selected)
                                   selected))))))
  (advice-add #'elfeed-search-selected :override #'maker/elfeed-search-selected))
#+end_src

Override ~elfeed-db-compact~ to bypass the projectile cache.
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-db-compact ()
...
#+begin_src elisp
(after! elfeed
  (cl-defun maker/elfeed-search-add-separators (&key (min-group-size 2))
...
inserted. "
  ...
  (defun maker/elfeed-search-post-process ()
    (maker/elfeed-search-add-separators :min-group-size 1))

  (defun maker/elfeed-forward-day ()
    "Move forward to the next day."
    (interactive)
    (let ((o (ov-in-next 'type 'date-separator)))
      (if o (goto-char (ov-end o)))))
  (defun maker/elfeed-backward-day ()
    "Move backward to the previous day."
    (interactive)
    (let ((o (ov-in-prev 'type 'date-separator)))
      (if o (goto-char (ov-end o)))))

  (add-hook 'elfeed-search-update-hook #'maker/elfeed-search-post-process))
#+end_src
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-search-entry< (a b)
               ...
               (compare-days (a b)
                             (let* ((a-day (day a))
                               ...
                               (if (= a-day b-day)
                                   ...
                                   (cl-case (maker/elfeed-search-unread< a-tags b-tags)
                                     (< nil)
                                     (> 't)
                                     (= ;; Same unread status; compare feed title
                                      ;; Same day: compare tags, then domain, then timestamp
                                      (cl-case (maker/elfeed-search-tags< a-tags b-tags)
                                        (< nil)
                                        (> 't)
                                        (= ;; Same tags; compare feed title
                                         (cl-case (maker/elfeed-search-feed< a b)
                                           (< nil)
                                           (> 't)
  ...
  (defun maker/elfeed-search-unread< (a-tags b-tags)
  ...
  (defun maker/elfeed-search-tags< (a-tags b-tags)
  ...
  (defun maker/elfeed-search-feed< (a b)
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-toggle-sort ()
    "Toggle the advanced sort function."
    (interactive)
    (if (not elfeed-search-sort-function)
        (setq elfeed-search-sort-function #'maker/elfeed-search-entry<
              elfeed-sort-order 'ascending)
...
#+begin_src elisp
(after! elfeed
  (defmacro maker/elfeed-search-at-entry (entry &rest body)
  ...
  (defun maker/elfeed-search-excerpt-toggle-selected (&optional hide-all)
...
With prefix, hide all excerpts."
    ...
    (if hide-all
      ...
      (--each (elfeed-search-selected)
        (maker/elfeed-search-at-entry it (maker/elfeed-excerpt-toggle)))))

  (defun maker/elfeed-excerpt-toggle ()
    (interactive)
    (or (maker/elfeed-excerpt-hide)
        (maker/elfeed-excerpt-insert)))

  (defun maker/elfeed-excerpt-hide ()
    (interactive)
  ...
  (defun maker/elfeed-wrap-string (string length)
  ...
  (defun maker/elfeed-excerpt-insert ()
    ...
    (when-let* ((pos (1+ (line-end-position)))
                ...
                (excerpt (--> content
                              (with-temp-buffer
                                ...
                                (buffer-string))
                              (maker/elfeed-wrap-string it width)
                              (concat it "\n")
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-search-mark-all-read ()
  ...
  (defun maker/elfeed-search-mark-all-unread ()
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-search-toggle-tag-group (tag predicate)
  ...
  (defun maker/elfeed-search-toggle-day-site-as-read ()
    "Mark all entries as read in the current site and day at point."
    (interactive)
    (maker/elfeed-search-toggle-tag-group 'unread (lambda (entry)
                                                     (list (time-to-days (seconds-to-time (elfeed-entry-date entry)))
                                                           (elfeed-entry-feed entry)))))
  (defun maker/elfeed-search-toggle-site-as-read ()
    "Mark all entries as read in the current site and day at point."
    (interactive)
    (maker/elfeed-search-toggle-tag-group 'unread (lambda (entry)
                                                     (elfeed-entry-feed entry))))
  (defun maker/elfeed-search-toggle-day-as-read ()
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-search-toggle-unread ()
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-search-toggle-star ()
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-delete-entries (pred)
...
PRED is called with one argument, the entry."
  ...
  (defun maker/elfeed-search-delete-selected ()
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-delete-old-entries (&optional before)
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-dead-feeds (&optional years)
...
(unpackaged/elfeed-search-view-hydra-define
  maker/elfeed-search-filter-hydra
  (:foreign-keys warn)
  ("Filter"
...
#+begin_src elisp
(after! elfeed
  (defvar maker/elfeed-outdated-tagger
    (elfeed-make-tagger :before "2 weeks ago"
                        :remove 'unread))

  (defvar maker/elfeed-odysee-marker
...
(map! :after elfeed
      ...
      :n  "C-j"        #'maker/elfeed-forward-day
      :n  "C-k"        #'maker/elfeed-backward-day
      :n  "<S-return>" #'maker/elfeed-search-excerpt-toggle-selected
      ...
      :n  "s"          #'maker/elfeed-toggle-sort
      :n  "f"          #'elfeed-search-live-filter
      :n  "F"          #'elfeed-search-clear-filter
      :n  "M-f"        #'maker/elfeed-search-filter-hydra/body
      :nv "m"          #'maker/elfeed-search-toggle-star
      :nv "u"          #'maker/elfeed-search-toggle-unread
      :n  "i"          #'maker/elfeed-search-toggle-day-site-as-read
      :n  "I"          #'maker/elfeed-search-toggle-site-as-read
      :n  "o"          #'maker/elfeed-search-toggle-day-as-read
      :n  "a"          #'maker/elfeed-search-mark-all-read
      :n  "A"          #'maker/elfeed-search-mark-all-unread
      :nv "d"          #'maker/elfeed-search-delete-selected
...
#+begin_src elisp
(after! elfeed-org
  (defadvice! +maker/consistent-elfeed-org-buffers (fn &rest args)
...
(after! elfeed-org
  ...
  (advice-add #'+rss-cleanup-h :override #'maker/+rss-cleanup-h))
#+end_src
...
(use-package! elfeed-dashboard
  ...
  (defun maker/elfeed-dashboard-all ()
...
(after! elfeed-dashboard
  ...
  (advice-add #'elfeed-dashboard :override #'maker/elfeed-dashboard)
  ...
  (add-hook! elfeed-dashboard-mode
    (defun maker/elfeed-dashboard-setup ()
  ...
  (add-hook! elfeed-dashboard-mode
    (defun maker/elfeed-dashboard-top ()
      (goto-line 3)))
  ...
  (defun maker/elfeed-dashboard-emphasis ()
  ...
  (defun maker/elfeed-dashboard-edit ()
    "Edit dashboard."
    (interactive)
    (+org-pretty-mode 0)
  ...
  (advice-add #'elfeed-dashboard-edit :override #'maker/elfeed-dashboard-edit)

  ;; Do not enable whitespace-mode in elfeed-dasboard (tabs used for icon alignment)
  (defadvice! maker/elfeed-dashboard-prevent-whitespace-mode (fn &rest args)
...
#+begin_src elisp
(after! elfeed-dashboard
  (defun maker/elfeed-dashboard-open (&optional arg)
...
#+begin_src elisp
(after! elfeed-dashboard
  (defun maker/elfeed-dashboard-update ()
    "Fetch new feeds, if there are no updates already in progress."
    (interactive)
    (unless elfeed-dashboard--elfeed-update-timer
      (elfeed-update)))
  (advice-add #'elfeed-dashboard-update :override #'maker/elfeed-dashboard-update)

  (add-hook! 'elfeed-update-init-hooks
    (defun maker/elfeed-update-info ()
...
#+begin_src elisp
(after! elfeed-dashboard
  (defun maker/elfeed-dashboard-parse-keymap ()
...
to group keymaps at the same place."
  ...
  (advice-add #'elfeed-dashboard-parse-keymap :override #'maker/elfeed-dashboard-parse-keymap))
#+end_src
...
(map! :after elfeed-dashboard
      ...
      (:prefix-map ("o" . "open")
       ...
       :desc "Elfeed (all)" "N" #'maker/elfeed-dashboard-all))
#+end_src
...
(use-package! elfeed-goodies
  ...
  (defun maker/elfeed-goodies/setup ()
    ...
    (setq elfeed-search-header-function #'elfeed-goodies/search-header-draw
          elfeed-search-print-entry-function #'maker/elfeed-goodies/entry-line-draw
  ...
  (advice-add #'elfeed-goodies/setup :override #'maker/elfeed-goodies/setup)

  (maker/elfeed-goodies/setup))
#+end_src
...
(after! elfeed-goodies
  ...
  (defface maker/elfeed-search-shown-face '((t (:background "gray23")))
...
(after! elfeed-goodies
  ...
  (defun maker/elfeed-goodies/search-header/rhs (separator-left separator-right search-filter stats update)
    (list
  ...
  (defun maker/elfeed-goodies/search-header/draw-wide (separator-left separator-right search-filter stats db-time)
  ...
  ;; NOTE Changed Subject -> Title
  (defun maker/elfeed-goodies/search-header/draw-tight (separator-left separator-right search-filter stats db-time)
  ...
  (advice-add #'search-header/rhs :override #'maker/elfeed-goodies/search-header/rhs)
  (advice-add #'search-header/draw-wide :override #'maker/elfeed-goodies/search-header/draw-wide)
  (advice-add #'search-header/draw-tight :override #'maker/elfeed-goodies/search-header/draw-tight))
#+end_src

Patch the header drawing function for ~elfeed-show-mode~.
#+begin_src elisp
(after! elfeed-goodies
  (defun maker/elfeed-goodies/entry-header-line ()
    "Print the header line for elfeed-show-mode."
    (cl-flet ((tags (entry)
                    (seq-difference (--map (substring-no-properties (symbol-name it))
                                           (elfeed-entry-tags entry))
                                    maker/elfeed-hidden-tags)))
      (let* (;; (entry-face 'elfeed-search-feed-face)
             ;; (entry-face (maker/elfeed-search--entry-face elfeed-show-entry))
        ...
        (concat
         ...
         (powerline-fill 'mode-line (powerline-width rhs))
         (powerline-render rhs)))))
  (advice-add #'elfeed-goodies/entry-header-line :override #'maker/elfeed-goodies/entry-header-line))
#+end_src
...
#+begin_src elisp
(after! elfeed-goodies
  (defun maker/elfeed-search--entry-face (entry)
...
#+begin_src elisp
(after! elfeed-goodies
  (defun maker/elfeed-search--entry-status (entry)
  ...
  (defun maker/elfeed-search--entry-icon (entry)
...
#+begin_src elisp
(after! elfeed-goodies
  (defun maker/elfeed-goodies/entry-line-draw (entry)
    ...
    (cl-flet ((add-faces (str &rest faces)
      ...
      (let* (;; Choose color and faces first
             ...
             (entry-face (maker/elfeed-search--entry-face entry))
             (entry-status (maker/elfeed-search--entry-status entry))
             (entry-icon (maker/elfeed-search--entry-icon entry))
  ...
  (advice-add #'elfeed-goodies/entry-line-draw :override #'maker/elfeed-goodies/entry-line-draw))
#+end_src
...
#+begin_src elisp
(after! elfeed
  (defun maker/elfeed-normalize-tags (tags &rest more-tags)
    "Return the normalized tag list for TAGS."
    (let ((all (apply #'append tags (nconc more-tags (list ())))))
      (cl-delete-duplicates all)))
  (advice-add #'elfeed-normalize-tags :override #'maker/elfeed-normalize-tags))
#+end_src
...
(after! ivy-posframe
  (setq ivy-posframe-min-height ivy-height
        ...
        ivy-posframe-size-function #'maker/ivy-posframe-get-size
  ...
  (defun maker/ivy-posframe-get-size ()
...
#+begin_src elisp
;; Fix regression in emacs 29.1: error Eager macro-expansion failure: (void-variable hydra-ivy/heads)
...
(after! org
  ...
  (add-hook 'org-cycle-hook
            (defun maker/org-cycle-align-tags (state)
...
(after! org-capture
  ...
  (defun maker/counsel-org-project-capture ()
    "Capture something into a project."
    (interactive)
    (maker/org-project-capture-ad #'counsel-org-capture)))
#+end_src
...
(after! org
  (map! (:leader
         ...
         (:prefix "p"
          ...
          "X" #'maker/counsel-org-project-capture))
...
The special symbol `none' means to not match headlines with any context tag.")
  ...
  (defvar +maker/org-view-groups-backlog
    '((:name "🎯 Goals" :transformer #'+maker/org-view-goals-transformer :todo nil)
      (:auto-outline-path t))
...
#+begin_src elisp
(after! org-ql
  (defadvice! maker/org-ql-view-prefix-format (&rest _)
