...
Define default span and starting day for daily/weekly agenda.
#+begin_src elisp
(after! org
...
Show blocked tasks with the dimmed face, customized.
These are usually projects and are useful to provide context in the backlog.
#+begin_src elisp
(after! org
...
Add extra files to agenda text search commands.
#+begin_src elisp
(after! org
...
#+begin_src elisp
(after! org
  (defun +org-agenda-filter-by-category (category exclude)
    "Filter lines in the agenda buffer that have the given category.
With a `\\[universal-argument]' prefix argument, exclude the lines of that category.
When there is already a (non-excluding) category filter in place, this command removes the filter."
    (interactive "P")
    (if (and org-agenda-filtered-by-category
             org-agenda-category-filter)
        (org-agenda-filter-show-all-cat)
      (cond
       ((and category exclude)
        (org-agenda-filter-apply
         (push (concat "-" category) org-agenda-category-filter) 'category))
       (category
        (org-agenda-filter-apply
  ...
  (defun +org-agenda-filter-astral ()
    "Remove entries with the Astral category from the agenda."
    (interactive)
    (if org-agenda-category-filter
        (org-agenda-filter-show-all-cat)
      (+org-agenda-filter-by-category "Astral" 't))))
#+end_src
...
(after! evil-org-agenda
  (map! :map evil-org-agenda-mode-map
        ...
        ;; Add equivalence for org-mode bindings (HJKL equivalents)
        :m "C-M-k"    #'org-agenda-priority-up
        :m "C-M-j"    #'org-agenda-priority-down
        :m "C-M-h"    #'org-agenda-do-date-earlier
        :m "C-M-l"    #'org-agenda-do-date-later
...
- [X] Need a fixed window width with truncate lines
- [ ] Use different display functions for some commands (e.g. swiper)
  - Supposed to work but does not
  - Keep it like this for now, let's see if ivy-posframe is really usable first.
#+begin_src elisp
(after! ivy-posframe
...
(after! pretty-hydra
  (map! :leader
        :desc "Adjust font size"   "-"   #'+hydra/font-size/body

        :prefix "w"
        :desc "Window management"  "SPC" #'+hydra/window-management/body
        :prefix "b"
        :desc "Buffer management"  "SPC" #'+hydra/buffer-management/body
))
#+end_src
...
[[https://github.com/jerrypnz/major-mode-hydra.el#pretty-hydra][This package]] provides a macro for creating hydras with automatically formatted columns.
#+begin_src elisp
