...
var TilingController = (function () {
    ...
    TilingController.prototype.onShortcut = function (ctx, input, data) {
        ...
        switch (input) {
            case Shortcut.Up:
                this.engine.cycleFocus(ctx, -1);
                break;
            case Shortcut.Down:
                this.engine.cycleFocus(ctx, +1);
                break;
            case Shortcut.FocusUp:
                this.engine.moveFocus(ctx, "up");
                break;
            case Shortcut.FocusDown:
                this.engine.moveFocus(ctx, "down");
                break;
            case Shortcut.FocusLeft:
                this.engine.moveFocus(ctx, "left");
                break;
            case Shortcut.FocusRight:
                this.engine.moveFocus(ctx, "right");
                break;
            ...
            case Shortcut.SwapUp:
                this.engine.moveTileDirection(ctx, "up");
                break;
            case Shortcut.SwapDown:
                this.engine.moveTileDirection(ctx, "down");
                break;
            case Shortcut.SwapLeft:
                this.engine.moveTileDirection(ctx, "left");
                break;
            case Shortcut.SwapRight:
                this.engine.moveTileDirection(ctx, "right");
                break;
...
var TilingEngine = (function () {
    ...
    TilingEngine.prototype.cycleFocus = function (ctx, step) {
        var window = ctx.currentWindow;
        if (window === null) {
    ...
    TilingEngine.prototype.moveFocus = function (ctx, dir) {
        var window = ctx.currentWindow;
        if (window === null) {
            var tiles = this.windows.getVisibleTiles(ctx.currentSurface);
            if (tiles.length > 1)
    ...
    TilingEngine.prototype.moveTileDirection = function (ctx, dir) {
        var window = ctx.currentWindow;
        if (window === null) {
            var tiles = this.windows.getVisibleTiles(ctx.currentSurface);
            if (tiles.length > 1)
...
var ThreeColumnLayout = (function () {
    ...
    ThreeColumnLayout.prototype.adjust = function (area, tiles, basis, delta) {
        ...
        if (tiles.length === 0)
            ...
            LayoutUtils.adjustAreaWeights(area, tiles.map(function (tile) { return _this.weights.get(tile); }), CONFIG.tileLayoutGap, tiles.indexOf(basis), delta).forEach(function (newWeight, i) {
                return _this.weights.set(tiles[i], newWeight * tiles.length);
            });
        }
            ...
            if (basisIndex < this.masterSize) {
                ...
                LayoutUtils.adjustAreaWeights(area, masterTiles_1.map(function (tile) { return _this.weights.get(tile); }), CONFIG.tileLayoutGap, basisIndex, delta).forEach(function (newWeight, i) {
                    return _this.weights.set(masterTiles_1[i], newWeight * masterTiles_1.length);
                });
            }
        }
        else if (tiles.length > this.masterSize + 1) {
    ...
    ThreeColumnLayout.prototype.apply = function (ctx, tileables, area) {
        ...
        if (tiles.length <= this.masterSize) {
            ...
            LayoutUtils.splitAreaWeighted(masterArea, masterTiles_2.map(function (tile) { return _this.weights.get(tile); }), CONFIG.tileLayoutGap)
                .forEach(function (tileArea, i) {
